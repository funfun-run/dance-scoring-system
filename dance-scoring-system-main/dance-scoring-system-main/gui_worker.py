# gui_worker.py - 后台线程封装，避免阻塞GUI

import threading
import os
from split_8beats import (
    get_video_info, detect_beats_from_audio, detect_beats_from_motion,
    calculate_segments_fixed, extract_slow_segment, merge_videos,
    BEATS_PER_SEGMENT, SLOW_SPEED
)
...
class Worker:
    ...
    def __init__(self, on_progress=None, on_done=None):
        ...
        self.on_done = on_done          # (success, result, error_message)
    ...
    def _run(self):
        try:
            ...
            if result is not None:
                ...
                from split_8beats import get_beat_segments
...
class ScoreWorker(Worker):
    ...
    def _run(self):
        try:
            from score_dance import Config, PoseExtractor, Scorer, download_model
            ...
            files = scorer.extract_clips_from_segments(segs, self.segments_dir)
