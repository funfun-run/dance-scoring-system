...
### scripts/score.py（v1.0）
- **统一分段逻辑**：移除旧的 `_changes_adaptive` / `_seg_adaptive` 自适应分段，改为 `_seg_by_beats` 固定 BPM 八拍分段，段号与 `scripts/split.py` 输出完全一致
- **新增 `-b` / `--bpm` 参数**：支持指定 BPM，默认 120，需与 `scripts/split.py` 保持一致
- **移除 `kp2d` 图像坐标**：`PoseFrame` 不再存储 2D 图像关键点，仅保留世界坐标 `kp3d`，简化数据结构
- **移除 `KP_WEIGHTS`**：关键点位置权重矩阵移除，评分仅依赖关节角度 (ANGLE_WEIGHTS)
- **自包含兜底提取**：当 `scripts/split.py` 的输出目录不存在或为空时，自动从参考视频实时提取低分/不合格片段，无需手动预处理
- **简化评分档位**：7 档分布统计合并为 5 档判定（优秀/良好/需改进/需重练/不合格）

### scripts/split.py（v1.0）
...
- **分段评分**：按固定 BPM 八拍等分分段（与 `scripts/split.py` 统一），每段判定合格（≥60）/ 不合格（<60），5 档判定输出总评
- **非线性评分**：3° 容差内满分，3°–15° 按 1.8 分/度扣分，15° 以上按 3.0 分/度加速扣分
- **慢动作练习**：优先输出不合格段，其次输出低于自定义阈值的片段。优先复用 `scripts/split.py` 的输出，缺失时自动从参考视频实时提取
...
> **Note**: 首次运行 `scripts/score.py` 会自动下载 MediaPipe 姿态模型（~5.6 MB），无需额外配置。
